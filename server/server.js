...
app.post('/api/download_journals', async (req, res) => {
  try {
    const response = await axios.post(`${pythonServiceUrl}/download_journals`, req.body);
    res.status(response.status).json(response.data);
  } catch (err) {
    res.status(err.response?.status || 500).json({ error: err.message, details: err.response?.data });
  }
});
app.post('/api/process_markdown', async (req, res) => {
  try {
    const response = await axios.post(`${pythonServiceUrl}/process_markdown`, req.body);
    res.status(response.status).json(response.data);
  } catch (err) {
    res.status(err.response?.status || 500).json({ error: err.message, details: err.response?.data });
  }
});
app.post('/api/download_pdfs', async (req, res) => {
  try {
    const response = await axios.post(`${pythonServiceUrl}/download_pdfs`, req.body);
    res.status(response.status).json(response.data);
  } catch (err) {
    res.status(err.response?.status || 500).json({ error: err.message, details: err.response?.data });
  }
});
app.post('/api/pdf_to_audio', async (req, res) => {
  try {
    const response = await axios.post(`${pythonServiceUrl}/pdf_to_audio`, req.body);
    res.status(response.status).json(response.data);
  } catch (err) {
    res.status(err.response?.status || 500).json({ error: err.message, details: err.response?.data });
  }
