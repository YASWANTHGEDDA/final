...
def convert_pdf_to_audio_api(pdf_file, chunk_duration_min=30, overlap_seconds=5):
    """
    Converts a single PDF to audio chunks and returns a dict with number of chunks, output file names, and errors.
    """
    try:
        base_name = os.path.splitext(pdf_file)[0]
        words_per_chunk = chunk_duration_min * WORDS_PER_MIN
        ...
        chunks = list(split_text_into_chunks_gen(text_iter, words_per_chunk, overlap_words))
        if not chunks:
            return {"status": "error", "error": "Empty or unreadable PDF.", "files": []}
        output_files = []
        for idx, chunk in enumerate(chunks):
            output_filename = f"{base_name}_part{idx + 1}.mp3"
            convert_text_to_audio(chunk, output_filename)
            output_files.append(output_filename)
        return {"status": "success", "num_chunks": len(chunks), "files": output_files}
