...
def download_pdfs_api(max_downloads=50, queries=None, api_key=None):
    """
    Downloads PDFs based on queries. Returns a summary dict for API use.
    """
    if api_key is None:
        api_key = os.environ.get('GEMINI_API_KEY')
    if not api_key:
        return {"error": "Gemini API key not provided. Set GEMINI_API_KEY env variable or pass api_key param."}
    ...
    base_query = "Algorithmic trading using Python textbooks and lecture notes filetype:pdf"
    query_variants = queries if queries else [
        base_query,
        "Python algorithmic trading textbooks filetype:pdf",
        "Algorithmic trading Python lecture notes filetype:pdf",
        "Quantitative finance Python textbooks filetype:pdf",
    ...
    while total_downloads < max_downloads and query_index < len(query_variants):
        query = query_variants[query_index]
        web_links = web_search_agent(query, popular_domains)
        arxiv_links = arxiv_search_agent(query)
        all_links = web_links + arxiv_links
                ...
                if arxiv_id:
                    ...
                    if filepath not in downloaded_files:
                        if download_arxiv_pdf(arxiv_id, filename):
                            ...
                            errors.append(f"Failed to download arXiv PDF: {arxiv_id}")
                else:
                    filename = f"pdf_{total_downloads}_{i}.pdf"
                    filepath = os.path.join(pdfs_folder, filename)
                    if filepath not in downloaded_files:
                        if download_pdf(link, f"{total_downloads}_{i}"):
                            ...
                            errors.append(f"Failed to download PDF: {link}")
            else:
                filename = f"pdf_{total_downloads}_{i}.pdf"
                filepath = os.path.join(pdfs_folder, filename)
                if filepath not in downloaded_files:
    ...
    summary = {
        ...
        "files": all_files,
        "errors": errors,
        "status": "success" if total_downloads > 0 else "no_files"
    }
    return summary

if __name__ == "__main__":
    result = download_pdfs_api()
    if "error" in result:
        print(result["error"])
    else:
