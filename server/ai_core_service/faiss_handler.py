...
class QueryProcessor:
    ...
    def expand_query(self, query):
        ...
        tech_expansions = {
            ...
            'neural': ['neural network', 'deep learning', 'artificial neural network'],
            'algorithm': ['algorithm', 'method', 'procedure', 'technique'],
            'optimization': ['optimization', 'minimization', 'maximization', 'improvement'],
            'analysis': ['analysis', 'examination', 'study', 'investigation'],
            'implementation': ['implementation', 'execution', 'deployment', 'realization']
        }
        
        for term, expansions_list in tech_expansions.items():
...
def query_index(user_id, query_text, k=3, active_file=None):
    """Enhanced query function with better accuracy and performance optimizations."""
    ...
    try:
        ...
        if config.ENABLE_QUERY_EXPANSION:
            expansions = query_processor.expand_query(query_text)
            expanded_queries.extend(expansions[:2])  # Limit to 2 additional queries
        
        user_index = None
        default_index = None

        # Query User Index with enhanced processing
        try:
