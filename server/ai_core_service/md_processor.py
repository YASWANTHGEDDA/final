...
def create_doc(slides_data, doc_filename, content_key):
    ...
    for slide in slides_data:
        content = slide.get(content_key, "")
        if content:
            document.add_paragraph(content)
    document.save(doc_filename)
    print(f"DOCX file '{doc_filename}' created successfully.")
    return doc_filename
...
"""
    ...
    pptx_file = create_ppt(slides_data, filename="Presentation.pptx")
    doc_file = create_doc(slides_data, "ImagePrompt.DOC", "image_prompt") # Image prompts usually don't have rich markdown
    create_doc(slides_data, "OtherNotes.DOC", "author_notes")

if __name__ == "__main__":
    # Example usage (optional):
    md = """### Slide 1:\n**Slide Text Content:** Example\n**Image Prompt:** Example\n---\n### Slide 2:\n**Slide Text Content:** Example2\n**Image Prompt:** Example2"""
    slides = refined_parse_markdown(md)
    pptx_file = create_ppt(slides, filename="test_presentation.pptx")
    doc_file = create_doc(slides, "test_doc.docx", "image_prompt")
    print(f"Created: {pptx_file}, {doc_file}")
