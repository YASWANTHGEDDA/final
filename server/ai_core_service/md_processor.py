...
def process_markdown_file(input_file, generate_files=True):
    """
    Processes a markdown file, parses slides, and optionally generates PPTX and DOCX files.
    Returns a summary dict for API use.
    """
    try:
    ...
    summary = {
        ...
        "slide_titles": [slide["title"] for slide in slides_data],
        "status": "success"
    }
    if not slides_data:
        summary["warning"] = "No slide data could be parsed from the input file. Output files will reflect this."
    if generate_files:
        create_ppt(slides_data, filename="Presentation.pptx")
        create_doc(slides_data, "ImagePrompt.DOC", "image_prompt")
        create_doc(slides_data, "OtherNotes.DOC", "author_notes")
    return summary

def main():
    input_file = "input.txt"
    result = process_markdown_file(input_file, generate_files=True)
    if "error" in result:
        print(result["error"])
    else:
        print(f"Processed {result['slide_count']} slides.")
        print("Slide titles:", result["slide_titles"])
