...
def generate_chat_response_route():
    ...
    logger.info(f"\n--- Received request at /generate_chat_response (ID: {request_id}) ---")
    if not request.is_json: return create_error_response("Request must be JSON", 400)
    data = request.get_json()
    if data is None: return create_error_response("Invalid or empty JSON body", 400)
    ...
    try:
        ...
        elapsed_time = time.time() - start_time
        logger.error(f"Request {request_id} failed with ConnectionError after {elapsed_time:.2f}s: {e}")
        return create_error_response(str(e), 502)
    except Exception as e: 
        elapsed_time = time.time() - start_time
        logger.error(f"Request {request_id} failed after {elapsed_time:.2f}s: {e}")
