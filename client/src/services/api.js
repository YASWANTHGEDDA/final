...
// --- TOOL ENDPOINTS ---
export const downloadJournals = (query, pageSize = 10, maxPages = 2) =>
  api.post('/download_journals', { query, page_size: pageSize, max_pages: maxPages });

export const processMarkdown = (inputFile, generateFiles = true) =>
  api.post('/process_markdown', { input_file: inputFile, generate_files: generateFiles });

export const downloadPdfs = (query, maxDownloads = 5, apiKey) =>
  api.post('/download_pdfs', { queries: query, max_downloads: maxDownloads, api_key: apiKey });

export const pdfToAudio = (pdfFile, chunkDurationMin = 30, overlapSeconds = 5) =>
  api.post('/pdf_to_audio', { pdf_file: pdfFile, chunk_duration_min: chunkDurationMin, overlap_seconds: overlapSeconds });
